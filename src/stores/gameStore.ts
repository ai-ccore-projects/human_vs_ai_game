...
interface GameStore extends GameState {
  /** ✅ unique id for the current game run (used for idempotent score saves) */
  gameId: UUID | null;
  setGameId: (id: UUID) => void;
...
function buildGameStore() {
  ...
  persist(
    (set, get) => ({
      ...
      gameId: null,
      setGameId: (id: UUID) => set({ gameId: id }),
      ...
      goToAttractMode: () =>
        set({ screen: 'attract', isPlaying: false, gameStarted: false, gameEnded: false }),
      ...
      startNewGame: () => {
        ...
        set({
          // reset the GameState slice
          ...initialGameState,
          // ✅ assign a fresh gameId per run
          gameId: newUuid(),
          screen: 'game',
          isPlaying: true,
          gameStarted: true,
      ...
      endGame: () => {
        ...
        if (isNewHighScore) set({ highScore: state.score });

        // Keep your local leaderboard behavior
        if (state.playerName) {
          get().addHighScore({
      ...
      resetGame: () => {
        ...
        set({
          ...initialGameState,
          // optionally clear gameId (or keep last one)
          gameId: null,
          playerName: state.playerName,
      ...
      incrementCombo: () =>
        set((s) => ({ combo: s.combo + 1, maxCombo: Math.max(s.maxCombo, s.combo + 1) })),
      ...
      // Rounds
      nextRound: () =>
        set((s) => ({ round: s.round + 1, bestRound: Math.max(s.bestRound, s.round + 1) })),
